...
def put_file(file_path):
    global current_memory_usage,i,j
    file_size = os.path.getsize(file_path)
    with memory_usage_lock:#dal ima dovoljno memorije
...
def get_file(file_id, destination_path):
    if file_id not in file_registry:
        ...
        return False
    file = file_registry.get(file_id)
    status = file['status']
    if status == "ready":
        ...
        print(f"File with {file_id} is not ready!")
...
def list_files():
    ...
    for file_id, file_info in file_registry.items():
        ...
        print(f"\tParts: {file_info['parts']}")
        for part_id in file_info['parts']:
            if part_id in file_parts_registry:
...
def shutdown_system():#pre nego sto ugasi prgoram prvo ugasi sve niti
